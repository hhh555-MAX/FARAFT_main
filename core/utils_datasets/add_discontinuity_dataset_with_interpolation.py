...
class DiscontinuityDatasetV2(data.Dataset):
    def __init__(self, original_dataset, source_image_transform=None, target_image_transform=None, flow_transform=None,
    ...
    def __getitem__(self, index):
        ...
        inputs = []
        dict = self.original_dataset.__getitem__(index)
        inputs.append(dict['source_image'])
        inputs.append(dict['target_image'])
        final_flow = dict['flow_map']
        mask_valid_correspondences = dict['correspondence_mask']
        ...
        for i in range(nbr_perturbations):
            ...
            while coordinate_in_mask is False:
                ...
                j = j + 1
                if mask_valid_correspondences[y, x] or j > 10:
                    coordinate_in_mask = True
            mask = self.get_gaussian(shape, mu=[x, y], sigma=sigma)

            max = mask.max()
            mask = np.clip(2.0 / (max * mask + 0.000000001), 0.0, 1.0)
        ...
        output = {'source_image': inputs[0],
                  'target_image': inputs[1],
                  'correspondence_mask': mask_valid_correspondences.astype(bool) if
