...
def corner_loss(output, flow_gt, valid, ce_weight):
    """ Loss function defined over sequence of flow predictions """
    four_preds = output['four_point_predictions']
    top_left = torch.Tensor(flow_gt[:, :, 0, 0]).unsqueeze(1) #N, 1, 2
    top_right = torch.Tensor(flow_gt[:, :, 0, -1]).unsqueeze(1)
    bottom_left = torch.Tensor(flow_gt[:, :, -1, 0]).unsqueeze(1)
    bottom_right = torch.Tensor(flow_gt[:, :, -1, -1]) .unsqueeze(1)
    four_gt = torch.concatenate([top_left, top_right, bottom_left, bottom_right], dim=1) #N, 4, 2
    loss = 0
    loss = loss_function(four_preds, four_gt)
    mace = ((four_preds[-1] - four_gt)**2).sum(dim=-1).sqrt().mean(dim=-1).detach().cpu().numpy().mean()
    return loss, mace


def loss_function(four_points_pred, four_points_gt, threshold=1, loss='l1'):
    loss = 0
    sp_flag = 0
    for i in range(len(four_points_pred)):
        x = torch.abs(four_points_pred[i] - four_points_gt).mean()
        if x < threshold: sp_flag = 1
        if loss == 'l1':
            loss += x
        elif loss == 'l2':
            loss += x**2
        elif loss == 'speedupl1':
            loss += (x - sp_flag * (1/(x + 0.1)))
