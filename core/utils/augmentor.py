...
class FlowAugmentor:
    def __init__(self, crop_size, min_scale=-0.2, max_scale=0.5, do_flip=True, pwc_aug=False, color_arg=True):
    ...
    def spatial_transform(self, img1, img2, flow, valid=None):
        ...
        if np.random.rand() < self.spatial_aug_prob:
            ...
            if valid is not None:
                valid = cv2.resize(valid.astype(np.uint8), None, fx=scale_x, fy=scale_y, interpolation=cv2.INTER_LINEAR).astype(bool)
        if self.do_flip:
        ...
        if valid is not None:
            ...
            valid = valid & get_gt_correspondence_mask(flow)
        return img1, img2, flow, valid

    def __call__(self, img1, img2, flow, valid=None):
        if self.color_arg is True:
